 ...
 *  Support: dd/mm/yyyy, yyyy-mm-dd, dd-mm-yyyy, d/m/yyyy, d/m/yy */
function parseDate(str) {
  ...
  if (slashParts.length === 3) {
    let [d, m, y] = slashParts.map(s => parseInt(s, 10));
    ...
    Papa.parse(path, {
      ...
      complete: (results) => {
        ...
        const rows = cleanedData.map(row => {
          ...
          const rawDate = row['Tanggal Pengambilan'] || row['Tanggal'] || row['Date'] || '';
          const parsedDt = parseDate(rawDate);
          const normalDate = parsedDt ? fmtDate(parsedDt) : rawDate;

          // Cost allocation — coba beberapa kemungkinan nama kolom
          const costAlloc = row['Cost Allocation'] || row['Cost Alloc'] ||
                            row['CostAllocation'] || row['cost_allocation'] || '';

          return {
            date:            normalDate,
            item_code:       row['Kode Item']    || row['Item Code']  || '',
            item_name:       row['Deskripsi']    || row['Description']|| row['Item Name'] || '',
            machine:         row['Mesin (Area)'] || row['Mesin']      || row['Machine']   || '',
            qty:             parseInt(row['Qty'] || row['QTY'] || '0') || 0,
            requester:       row['Pemohon']      || row['Requester']  || '',
...
function initSettings() {
  // Load saved settings dulu sebelum attach listeners
  loadSettings();
  syncSettingsUI();
